...
/*/{Protheus.doc} FSFIN003
    (Função criada para gerar 
    boletos Santander
    empresa Agrogera 
    @type  Function
    @author Maria Luiza
...
User Function FSFIN003()
	...
	Private cFilter    := ''
	Private lMarca	   := .F.
	Private aBMP       := {} //"\system\Santander.bmp" //aBitMap
	...
	If nOpc == 1
		...
		If SEE->(DbSeek(xFilial("SEE")+mv_par15+cCodAge+cCodCta+"001"))
			If SA6->(DbSeek(xFilial("SA6")+mv_par15+cCodAge+cCodCta))
				If "SANTANDER" $ SA6->A6_NOME
					If !Empty(SA6->A6_NUMBCO)
						cBanco := Alltrim(SA6->A6_NUMBCO)
					EndIf
				Else
					llOk := .F.
					Aviso(OemToAnsi("ATENÇÃO"),OemToAnsi("O banco selecionado nos parâmetros não corresponde ao Banco do Santander!"),{"OK"})
				EndIf
			Else
				llOk := .F.
...
//aAdd(aBMP,"\system\Santander.bmp")
	...
	While SE1->(!EOF())
		...
		If Marked("E1_OK")//aMarked[i]
			...
			_nVlrAbat   +=  SE1->E1_DECRESC + SE1->E1_INSS + SE1->E1_CSLL + SE1->E1_COFINS + SE1->E1_PIS + _ImpRet   //E1_VALLIQ //SomaAbat(SE1->E1_PREFIXO,SE1->E1_NUM,SE1->E1_PARCELA,"R",1,,SE1->E1_CLIENTE,SE1->E1_LOJA)

...
Static Function fMontaFun()
    ...
    IF !("(" $ cNomeFun) .OR. !(")" $ cNomeFun)
        ...
        PREPARE ENVIRONMENT EMPRESA '01' FILIAL '01' USER 'Administrador' PASSWORD 'B@2602' TABLES "SC5","SC6","SA1","SA2","SB1","SB2","SF4" MODULO 'FAT' FUNNAME "'"+cNomeFun+"'"
        /*******COMANDOS *********/
        RESET ENVIRONMENT
    else
        cNomeFun := Alltrim(cNomeFun)

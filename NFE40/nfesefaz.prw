				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							SB1->B1_EX_NCM,;
							cD2Cfop,;
							IIF(SB1->B1_UM == "MZ","MWH",SB1->B1_UM),; //	                 SB1->B1_UM,;   RICARDO MOREIRA TRATAMENTO VENDA DE ENERGIA (11/07/19) 
							(cAliasSD2)->D2_QUANT,;
...
If Len(aICMSZFM) > 0 .And. Len(aCST) > 0 .And. !Empty(aICMSZFM[1])
	...
	Endif
Else
   cString += NfeTag('<vDesc>' ,ConvType(aProd[15],15,2)) //Ricardo 30/11/2021	
EndIf

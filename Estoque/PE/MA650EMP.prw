...
User Function MA650EMP  ////MTAGRSD4()  //MA650EMP
   ...
   /*Ajuste para gravar a descrição do produto nas OP's intermediárias
   Solicitado pelo Hugo #GLPI 14635 - Maria Luiza 13/02/2026 Início*/
   ...
   /*Ajuste para gravar a descrição do produto nas OP's intermediárias
   Solicitado pelo Hugo #GLPI 14635 - Maria Luiza 13/02/2026 Fim*/
	...
	DO WHILE TMP1->(!EOF())
		...
		DO WHILE TMP1->(!EOF()) .AND. _Op = TMP1->D4_OP
			...
			//Alteração para tirar o empenho da D4 (D4_QUANT) inicio
/*
      DbSelectArea("SD4")
      DbSetOrder(2)   
      IF dbSeek(xFilial("SD4")+TMP1->D4_OP+TMP1->D4_COD)
         ...
         SD4->(DbCloseArea())
	   EndIF	    
  //Alteração para tirar o empenho da D4 (D4_QUANT) fim
 */    
 ...
 /*     
      ...
      IF dbSeek(xFilial("SB2")+TMP1->D4_COD+TMP1->D4_LOCAL)    
         ...
         SB2->(DbCloseArea())
      EndIf
 */ //Comentado parte do programa dia 25/10/23-Jonathas-TI      
			If TMP1->C2_TPOP = 'F'
				DbSelectArea("SB1")
				DbSetOrder(1)  // Z42_FILIAL +  Z42_NUM
				IF dbSeek(xFilial("SB1")+TMP1->D4_PRODUTO)
					IF (SB1->B1_REQ == "N") .AND. VAL(TMP1->C2_SEQPAI) > 1 //!EMPTY(TMP1->C2_SEQPAI)    //.OR.    //IF (SB1->B1_REQ == "N") .AND. (!EMPTY(TMP1->C2_SEQPAI)
						DBGOTOP()
						DbSelectArea("SD4")
						DbSetOrder(2)
						IF dbSeek(xFilial("SD4")+TMP1->D4_OP+TMP1->D4_COD)

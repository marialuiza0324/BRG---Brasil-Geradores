...
//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	...
	OLE_SetDocumentVar(hWord,'cDtMun'		,AllTrim("Goianira") +", "+ SubStr(cDta,7,2) +" de "+ MesExtenso(Val(SubStr(cDta,5,2))) +" de "+ SubStr(cDta,1,4)+".")
	OLE_SetDocumentVar(hWord,'cTpFrete'		,iif(AD1->AD1_XTPFRE=="C","CIF","FOB") )
	//OLE_SetDocumentVar(hWord,'cDtEntrega'	,dToc(AD1->AD1_XFECEN))	
	OLE_SetDocumentVar(hWord,'cCondPagto'	,ADY->ADY_CONDPG + " - " + Posicione("SE4",1,FWxFilial("SE4")+ADY->ADY_CONDPG,"E4_DESCRI") )
	OLE_SetDocumentVar(hWord,'cValidade'	,dToc(AD1->AD1_DTFIM))  
	OLE_SetDocumentVar(hWord,'cVendedor'	,Posicione("SA3",1,FWxFilial("SA3")+AD1->AD1_VEND,"A3_NOME"))
	OLE_SetDocumentVar(hWord,'cEmail'		,Posicione("SA3",1,FWxFilial("SA3")+AD1->AD1_VEND,"A3_EMAIL"))
...
//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	...
	If dbSeek(xFilial("ADZ")+cProposta+cPRevisa)
		...
		While ADZ->(!Eof()) .and. xFilial("ADZ") == ADZ->ADZ_FILIAL .AND. cProposta == ADZ->ADZ_PROPOS
			...
			nI++  
			OLE_SetDocumentVar(hWord,"prt_item"+Alltrim(str(nI))  ,AllTrim(ADZ->ADZ_ITEM))
			OLE_SetDocumentVar(hWord,"prt_prod"+Alltrim(str(nI))  ,Alltrim( ADZ->ADZ_PRODUT) )
			//OLE_SetDocumentVar(hWord,"prt_desc"+Alltrim(str(nI))  ,Alltrim( Posicione("SB1",1,xFilial("SB1")+ADZ->ADZ_PRODUT,"B1_XOBS") ) )
			OLE_SetDocumentVar(hWord,"prt_um"+Alltrim(str(nI))  ,Alltrim( Posicione("SB1",1,xFilial("SB1")+ADZ->ADZ_PRODUT,"B1_UM") ) )
			OLE_SetDocumentVar(hWord,"prt_qtd"+Alltrim(str(nI))   ,Transform(ADZ->ADZ_QTDVEN,"999,999,999.99"))
			OLE_SetDocumentVar(hWord,"prt_vuni"+Alltrim(str(nI))  ,Transform(ADZ->ADZ_PRCVEN,"@E 999,999,999.99"))
			OLE_SetDocumentVar(hWord,"prt_vtot"+Alltrim(str(nI))  ,Transform(ADZ->ADZ_TOTAL,"@E 999,999,999.99"))
	...
	If nI > 0
		...
		OLE_SetDocumentVar(hWord,'prt_nroitens',alltrim(Str(nI)))
